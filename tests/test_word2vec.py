...
UserStorySimilarityWord2vec.load_model(logging.getLogger("testLogger")) # mock this still

@pytest.fixture
def usSimilarityVsm():
    logger = logging.getLogger("testLogger")
    mapper = FeedUvlMapper(logger) # mock this
    return UserStorySimilarityWord2vec(mapper, 0.1, False, False)
...
def test_measure_all_pairs_similarity(usSimilarityVsm: UserStorySimilarityWord2vec, us_dataset, expected_similarity_result_entry_1, expected_similarity_result_entry_2, expected_similarity_result_entry_3):
...
def test_measure_all_pairs_similarity_one_us(usSimilarityVsm: UserStorySimilarityWord2vec):
    result = usSimilarityVsm.measure_all_pairs_similarity(["I do not matter"])
    assert result == []

def test_measure_all_pairs_similarity_no_us(usSimilarityVsm: UserStorySimilarityWord2vec):
...
def test_measure_pairwise_similarity(usSimilarityVsm: UserStorySimilarityWord2vec, us_dataset, expected_similarity_result_entry_1, expected_similarity_result_entry_2):
...
def test_measure_pairwise_similarity_one_us(usSimilarityVsm: UserStorySimilarityWord2vec):
    result, unexistent_ids = usSimilarityVsm.measure_pairwise_similarity(["I do not matter"], ["COMET-183"], [])
    assert result == []
    assert unexistent_ids == 0

def test_measure_pairwise_similarity_no_us(usSimilarityVsm: UserStorySimilarityWord2vec):
    result, unexistent_ids = usSimilarityVsm.measure_pairwise_similarity([], ["COMET-183"], [])
    assert result == []
    assert unexistent_ids == 0

@pytest.mark.parametrize("us_dataset", [us_dataset])
def test_measure_pairwise_similarity_unexistent_id(usSimilarityVsm: UserStorySimilarityWord2vec, us_dataset):
    result, unexistent_ids = usSimilarityVsm.measure_pairwise_similarity(us_dataset, ["Unexistent-1"], [])
    assert len(result) == 0
    assert unexistent_ids == 1

@pytest.mark.parametrize("us_dataset", [us_dataset])
def test_measure_pairwise_similarity_unexistent_id_not_extracted(usSimilarityVsm: UserStorySimilarityWord2vec, us_dataset):
